...
with_iter = load("portfolio_iterations_1.00.jld","with_iter")
without_iter = load("portfolio_iterations_1.00.jld","without_iter")
first_iter_num = load("portfolio_iterations_1.00.jld","first_iter_num") 
total_nodes = load("portfolio_iterations_1.00.jld","total_nodes")
total_nodes_without = load("portfolio_iterations_1.00.jld","total_nodes_without")
println(total_nodes)
...
subplot(211)
p1, = PyPlot.step(ind , without_iter, color= "black", label="No early termination", marker ="o", markersize = 4, markevery = 1)
p2, = PyPlot.step(ind, with_iter, color= "red", label="With early termination", marker = "x", markersize = 4, markevery = 1)
...
xlim([1, lastindex(with_iter)])
subplot(212)

PyPlot.step(ind, ones(length(ind)), color= "black", marker = "o", markersize = 4)
PyPlot.step(ind, percentage, color= "red",marker = "x", markersize = 4)
ylabel("Ratio")
xlabel("Days")
xlim([1, lastindex(with_iter)])

savefig("portfolio_iterations_plot.pdf")
