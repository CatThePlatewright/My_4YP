...
for i in 1:lastindex(k_list)
    ...
    p2, = PyPlot.step(ind, with_iter, color= color_set[i], label = @sprintf("k= %d",i), marker = marker_set[i], markersize = 4, markevery = 1)
    annotate("Dashed - No early termination",(13,3.5))
    annotate("Solid - With early termination",(13,3))
...
savefig("toy_problem_3k_warmstart.pdf")
